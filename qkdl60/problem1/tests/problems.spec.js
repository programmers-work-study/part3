...
test.describe("게시물 목록 페이지 테스트", () => {
  test("TC 1. 게시물 목록이 올바르게 표시되는 기능이 적절하게 동작한다.", async ({page}) => {
  ...
  test("TC 2. 작성자 별 필터링 기능이 적절하게 동작한다.", async ({page}) => {
    ...
    const postCards = await page.$$("#postList div");
    const filteredPosts = await page.$$eval("#postList div h3", (nodes) => nodes.filter((node) => node.textContent.includes("작성자 1")));
  ...
  test("TC 5. 게시물에는 작성자 정보가 적절하게 포함되어있다.", async ({page}) => {
    await page.goto("./base");
    await page.waitForSelector("#postList div");
  ...
  test("TC 6. 작성자를 필터링하면 해당 작성자의 게시물만 표시된다.", async ({page}) => {
    await page.goto("./base");
    await page.waitForSelector("#postList div");
